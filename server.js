...
const app = express();
const LOGO_URL = "https://cdn.discordapp.com/attachments/1472499182479867915/1528203140820897823/IMG_4036.jpg";
const BG_URL = "https://cdn.discordapp.com/attachments/1472499182479867915/1528201915429683450/IMG_4034.png";
...
const guildSchema = new Schema({ guild: String, joins: Array, fbijoins: Array, joinChannels: Object, fbiListMessage: String }, { strict: false });
...
app.get('/api/me', async (req, res) => {
    ...
    try {
        ...
        if (character.id?.job !== 'fbi') return res.json({ loggedIn: true, registered: true, authorized: false, username: req.session.username, avatar: req.session.avatar });

        const onDuty = !!db?.fbijoins?.find(j => j.user === req.session.userId);
        ...
        return res.json({
            ...
            character: {
                ...
                rank: character?.id?.fbi_data?.rank || '',
                sector: character?.id?.fbi_data?.sector || '',
                code: character?.id?.fbi_data?.code || '',
...
app.post('/api/duty/login', async (req, res) => {
    ...
    try {
        ...
        if (character.id?.job !== 'fbi') return res.json({ success: false, message: 'هذا النظام للافباي فقط' });
        ...
        if (db.fbijoins?.find(j => j.user === req.session.userId)) return res.json({ success: false, message: 'أنت مباشر بالفعل' });

        if (!db.fbijoins) db.fbijoins = [];
        db.fbijoins.push({ user: req.session.userId, name: character.id.first, character: check.character });

        const pointsIndex = character.fbi_points?.findIndex(p => p.name === 'login');
        if (pointsIndex !== -1 && character.fbi_points) {
            character.fbi_points[pointsIndex].value = (character.fbi_points[pointsIndex].value || 0) + 4;
            userDoc.markModified(`characters.${check.character}.fbi_points`);
            await userDoc.save();
        }

        db.markModified('fbijoins');
        await db.save();
...
app.post('/api/duty/logout', async (req, res) => {
    ...
    try {
        const db = await guildBase.findOne({ guild: GUILD_ID });
        const index = db?.fbijoins?.findIndex(j => j.user === req.session.userId);
...
app.get("/api/mdt/:idNumber", async (req, res) => {
    ...
    try {
        ...
        if (!myChar || myChar.id?.job !== "fbi") return res.json({ success: false, message: "للافباي فقط" });
        if (!db.fbijoins?.find(j => j.user === req.session.userId)) return res.json({ success: false, message: "يجب تسجيل المباشرة أولاً" });
...
app.post("/api/priority/send", async (req, res) => {
    ...
    try {
        ...
        const joinCheck = db.fbijoins?.find(c => c.user === req.session.userId);
        ...
        if (!character || character.id?.job !== "fbi") return res.json({ success: false, message: "شخصيتك الحالية ليست وظيفة عسكرية" });
